...
class EloquentUsedVehicleRepository extends BaseRepository implements UsedVehicleRepository {
     ...
     * @param Vehicle $model
     */
    function __construct(UsedVehicle $model) {
        $this->model = $model;
    }

    public function attachThumbnail($used_vehicle, $thumbnail) {
        if (isset($thumbnail) && $thumbnail->isValid()) {
            ...
            $fileName = 'used-vehicle-thumbnail-' . str_pad($used_vehicle->id, 10, '0', STR_PAD_LEFT) . '.' . $extension; // renameing image
            $thumbnail->move($basePath . $destinationPath, $fileName); // uploading file to given path

            $used_vehicle->thumbnail = "$destinationPath/$fileName";
            $used_vehicle->save();
        }
        return $used_vehicle;
    }
     ...
     * @return mixed
     */
    public function attachPictures($used_vehicle, $pictures)
    {
        $counter = $used_vehicle->pictures()?$used_vehicle->pictures()->count():0;
        if(is_array($pictures)) {
            foreach($pictures as $picture) {
                if (isset($picture) && $picture->isValid()) {
                    ...
                    $fileName = 'VGP-'. $used_vehicle->id . '-' . milliseconds() . (++$counter) . '.' . $extension;
                    ...
                    $pictureEntity->caption = $used_vehicle->model;
                    $used_vehicle->pictures()->save($pictureEntity);
                }
            }
        }
        return $used_vehicle;
